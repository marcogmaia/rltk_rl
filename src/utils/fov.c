...
static unsigned height(fov_settings_type* settings, int x, unsigned maxdist) {
    unsigned** newheights;
...
#define FOV_DEFINE_OCTANT(signx, signy, rx, ry, nx, ny, nf, apply_edge,        \
                          apply_diag)                                          \
    static void fov_octant_##nx##ny##nf(fov_private_data_type* data, int dx,   \
        ...
        if(!apply_diag && dy1 == dx) {                                         \
            /* We do diagonal lines on every second octant, so they don't get  \
             * done twice. */                                                  \
            --dy1;                                                             \
        }                                                                      \
                                                                               \
        ...
        case FOV_SHAPE_CIRCLE:                                                 \
            h = (unsigned)sqrtf(                                               \
                (float)(data->radius * data->radius - dx * dx));               \
            break;                                                             \
        case FOV_SHAPE_OCTAGON: h = (data->radius - dx) << 1; break;           \
        default: h = data->radius; break;                                      \
        };                                                                     \
        if((unsigned)dy1 > h) {                                                \
            if(h == 0) {                                                       \
                return;                                                        \
        ...
        for(dy = dy0; dy <= dy1; ++dy) {                                       \
            ...
            if(settings->opaque(data->map, x, y)) {                            \
                if(settings->opaque_apply == FOV_OPAQUE_APPLY                  \
                   && (apply_edge || dy > 0)) {                                \
                    settings->apply(data->map, x, y, x - data->source_x,       \
                                    y - data->source_y, data->source);         \
                }                                                              \
                if(prev_blocked == 0) {                                        \
                    end_slope_next                                             \
                        = fov_slope((float)dx + 0.5f, (float)dy - 0.5f);       \
                    fov_octant_##nx##ny##nf(data, dx + 1, start_slope,         \
                                            end_slope_next);                   \
                }                                                              \
                prev_blocked = 1;                                              \
            } else {                                                           \
